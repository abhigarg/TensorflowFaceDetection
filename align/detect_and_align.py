...
def detect_faces(img, mtcnn):
    ...
    w2 = 400
    h2 = int(float(h) * float(w2) / float(w))

    resz_img = cv2.resize(img, (w2, h2))

    bounding_boxes, landmarks = detect_face(resz_img, mtcnn['pnet'], mtcnn['rnet'], mtcnn['onet'])
    ...
    if nrof_bb > 0:
        ...
        for i in range(nrof_bb):
            ...
            # det[0] = int(float(det[0]) * float(w) / float(w2))
            # det[2] = int(float(det[2]) * float(w) / float(w2))
            # det[1] = int(float(det[1]) * float(h) / float(h2))
            # det[3] = int(float(det[3]) * float(h) / float(h2))
            bb = np.zeros(4, dtype=np.int32)
            bb[0] = int(float(np.maximum(det[0] - margin / 2, 0)) * float(w) / float(w2))
            bb[1] = int(float(np.maximum(det[1] - margin / 2, 0)) * float(h) / float(h2))
            bb[2] = int(float(np.minimum(det[2] + margin / 2, w2)) * float(w) / float(w2))
            bb[3] = int(float(np.minimum(det[3] + margin / 2, h2)) * float(h) / float(h2))
